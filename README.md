...
Final placings are used only as the last tie-breaker. They are not generated by the data refresh because official tournament placings require tournament-specific judgement after knockout rounds.
...
Enter final placing corrections in `public/data/manual-overrides.json` only if the last tie-breaker is needed:

```json
{
  "teamResults": [
    { "name": "Argentina", "finalPlacing": 1 },
    { "name": "France", "finalPlacing": 2 }
  ]
}
```

The same file can also override a stage if the fixture source cannot infer it:

```json
{ "name": "Argentina", "stageReached": "winner", "finalPlacing": 1 }
```

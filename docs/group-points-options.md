...
2. For each team, calculate:
   ...
   - goal difference
   - group points
3. Sort each group by current standings rules.
4. Mark provisional qualifiers:
   - positions 1 and 2 in each group
   - best eight third-place teams across groups
5. Add `provisionalStage(teamName)` or `effectiveStage(teamName)`:
...
6. Use this effective stage inside `teamContribution()` when a `showGroupPoints` mode is enabled.
...
2. Write the calculated standings into `worldcup.json` alongside fixtures.
3. In `app.js`, read `state.worldcup.groups` instead of deriving tables in the browser.
...
6. Add CSS classes for provisional badges, for example `provisional`, `qualified`, `third-place`, and `eliminated`.

## Tie-break details

Use the best available deterministic tie-breaks from loaded fixture data:
...
> Group-position points are provisional and assume the current top two teams in each group, plus the eight best third-placed teams, reach the Round of 32. Official sweepstake points still come from confirmed knockout fixtures.
